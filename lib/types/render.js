 ...
 */
export const KEYBOARD_ACTION_ROWS = [
 ...
 */
export function formatStartClock(ms, now) {
    const d = new Date(ms);
 ...
 */
export function startClockLabel(ms, now) {
 ...
 */
export function isWorkspaceInstructionSource(source) {
 ...
 */
export function toolArgBrief(raw) {
    let brief = raw;
    try {
 ...
 */
export function toolCallBrief(name, raw) {
 ...
 */
export function reasoningBrief(text) {
 ...
 */
export function messageActions(message, time) {
    const actions = [];
    for (const block of message.content) {
        if (block.type === 'reasoning') {
            const brief = reasoningBrief(block.text);
            if (brief !== '')
                actions.push({ time, line: `${REASONING_ACTION_LABEL} — ${brief}` });
        }
 ...
 */
export function turnEndLabel(reason) {
    switch (reason.kind) {
...
export function emptyRoundUsage() {
 ...
 */
export function accumulateRoundUsage(usage, step) {
    if (step === undefined)
        return;
 ...
 */
export function compactTokenCount(count) {
    if (count < 1000)
        return String(count);
 ...
 */
export function roundUsageFooter(usage) {
    const parts = [];
 ...
 */
export function lastTurnUsage(events) {
    let startIndex = -1;
    let endIndex = -1;
    ...
    if (startIndex === -1 || endIndex <= startIndex)
        return emptyRoundUsage();
    const usage = emptyRoundUsage();
    for (let index = startIndex + 1; index < endIndex; index += 1) {
        const event = events[index];
        if (event?.type === 'assistant/message')
            accumulateRoundUsage(usage, event.data.usage);
    }
    return usage;
}
/**
 ...
 */
export function openTurnUsage(events) {
    let startIndex = -1;
    let endIndex = -1;
    for (let index = 0; index < events.length; index += 1) {
        const event = events[index];
 ...
 */
export function latestTurnStartTime(events) {
    let time;
 ...
 */
export function sessionGlyph(summary) {
 ...
 */
export function sessionRow(index, summary, now) {
 ...
 */
export function assistantTail(events, rounds) {
    const texts = [];
    for (const event of events) {
        if (event.type !== 'assistant/message')
            continue;
 ...
 */
export function statusMainText(events) {
    if (events.length === 0)
        return '（空白会话，还没有消息）';
    const openCalls = [];
    for (const event of events) {
 ...
 */
export function openToolCalls(events) {
    const open = [];
    for (const event of events) {
        switch (event.type) {
            case 'tool/call':
 ...
 */
export function turnOpen(events) {
    let started = false;
    for (const event of events) {
        if (event.type === 'turn/start')
            started = true;
 ...
 */
export function pendingAskBatches(events) {
 ...
 */
export function stepActions(events, maxActions = 20) {
    const actions = [];
    for (const event of events) {
        if (event.type === 'assistant/message')
            actions.push(...messageActions(event.data.message, event.time));
    }
 ...
 */
export function actionsHtml(actions, now) {
    if (actions.length === 0)
        return '';
 ...
 * @returns the `<blockquote expandable>` fragment, or ''.
 */
export function stepActionsHtml(events, now, maxActions = 20) {
 ...
 */
export function chunkText(text, cap = TELEGRAM_CHUNK_MAX) {
    const points = Array.from(text);
 ...
 */
export function stripStreamSuffix(text) {
 ...
 */
export function statusStats(events) {
    let users = 0;
    let assistants = 0;
    let tools = 0;
    let chars = 0;
    for (const event of events) {
        switch (event.type) {
            case 'user/message':
                ...
                if (isWorkspaceInstructionSource(event.data.source))
                    break;
                users++;
 ...
 */
export function sessionKeyboardRows(items, verb) {
 ...
 * @returns the reply-keyboard rows, each row a list of button texts.
 */
export function attachKeyboardRows(items) {
    const rows = [...KEYBOARD_ACTION_ROWS.map(row => [...row])];
 ...
 * @returns the two-line row text.
 */
export function workspaceRow(index, workspace) {
 ...
 */
export function workspaceKeyboardRows(workspaces) {
    const rows = [...KEYBOARD_ACTION_ROWS.map(row => [...row])];
...
export function todoRow(index, item) {
 ...
 */
export function lastTodoWrite(events) {
    let latest = null;
    for (const event of events) {
 ...
 * @returns the reply-keyboard rows.
 */
export function presetKeyboardRows(presets) {
    const rows = [...KEYBOARD_ACTION_ROWS.map(row => [...row])];
...
export function attachWorkspaceData(workspaceId) {
...
export function attachSessionData(sessionId) {
 ...
 */
export function parseAttachCallback(data) {
 ...
 */
export function attachScopeButtons(workspaces, options) {
    const rows = [];
    ...
    if (options.ungrouped)
        rows.push([{ text: '未分组', data: ATTACH_UNGROUPED_DATA }]);
    if (options.archived)
        rows.push([{ text: '归档', data: ATTACH_ARCHIVED_DATA }]);
    return rows;
}
/**
 ...
 * @returns the keyboard rows.
 */
export function attachSessionButtons(items) {
    const rows = [];
 ...
 * @returns the decoded action, or `undefined` for an unknown token.
 */
export function parseSessionListCallback(data) {
    const [prefix, id, ...rest] = data.split(':');
    if (id === undefined || id === '')
        return undefined;
    if (rest.length !== 0)
        return undefined;
    if (prefix === SESSION_LIST_CALLBACK_STOP)
        return { kind: 'stop', sessionId: SessionId(id) };
    if (prefix === SESSION_LIST_CALLBACK_STATUS)
        return { kind: 'status', sessionId: SessionId(id) };
    return undefined;
}
/**
 ...
 * @returns the keyboard rows.
 */
export function sessionActionButtons(items, verb) {
    const rows = [];
    for (const item of items.slice(0, SESSION_KEYBOARD_MAX)) {
        const title = item.projections?.values.title ?? item.cwd ?? shortSessionId(item.sessionId);
...
export function parseQuestionCallback(data) {
    ...
    if (prefix === QUESTION_CALLBACK_OPTION) {
        ...
        if (questionIndex === undefined || option === undefined)
            return undefined;
 ...
 */
export function questionMessageText(questions) {
 ...
 * @returns the keyboard rows.
 */
export function questionKeyboard(questions, selected, custom, rpcId) {
