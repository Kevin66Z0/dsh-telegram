...
import { SessionId, type SessionEvent, type SessionId as SessionIdBrand, type TodoItem, type TurnEndReason } from '@deepseek-ai/dsh-session'
...
import {
  ASSISTANT_ROLE_GLYPH, HISTORY_DEFAULT_LIMIT, KEYBOARD_ACTION_ROWS,
  QUEUE_ACK_MAX, SESSION_KEYBOARD_MAX, SESSION_STATE_LEGEND, STREAM_REPLYING_SUFFIX,
  TELEGRAM_CHUNK_MAX, TELEGRAM_VERSION, WORKSPACE_KEYBOARD_MAX,
  accumulateRoundUsage, assistantTail, attachScopeButtons, attachSessionButtons, blockText, chunkText, emptyRoundUsage, stripStreamSuffix,
  actionsHtml, messageActions, stepActionsHtml, turnOpen,
  parseAttachCallback, parseQuestionCallback, parseSessionListCallback, pendingAskBatches, presetKeyboardRows,
  questionKeyboard, questionMessageText, renderTodoList,
  attachKeyboardRows, lastTurnUsage, latestTurnStartTime, openTurnUsage, roundUsageFooter, sessionActionButtons, sessionKeyboardRows,
  startClockLabel, sessionRow, shortSessionId,
  statusMainText, statusStats, timeAgo,
  truncate, turnEndLabel, workspaceKeyboardRows, workspaceRow,
  type AnswerButton, type RoundUsage, type StepAction,
} from './render.ts'
...
export interface SessionConsolePort {
  ...
  listGlobalModels(): Promise<{ groups: ModelProviderGroup[]; failures: ModelCatalogFailure[] }>
   ...
   */
  createSession(options?: { workspaceId?: WorkspaceId; cwd?: string; agentPreset?: string }): Promise<SessionId>
...
/**
 * One reply session-list keyboard's re-emission recipe: the listed sessions
 * a refresh re-derives from the registry — the full visible /delete list, or
 * the /attach bucket it listed. (/stop and /status session lists are inline
 * and never tracked for re-emission.)
 */
type SessionListRecipe =
  | { list: 'full' }
  | { list: 'ungrouped' }
  | { list: 'archived' }
  | { list: 'workspace'; workspaceId: WorkspaceId }

/**
 * The reply keyboard a chat currently shows. Session lists keep their
 * emission hint and re-emission recipe so a delete or rename re-emits the
 * same list with the same hint; every other surface is an opaque marker, so
 * a refresh only ever targets a live session list.
 */
type VisibleKeyboard =
  | { kind: 'sessions'; hint: string; recipe: SessionListRecipe }
  | { kind: 'scopes' }
  | { kind: 'workspaces' }
  | { kind: 'model' }
  | { kind: 'presets' }
...
interface ChatState {
  /** The bound session this chat sends prompts to; undefined when none. */
  sessionId: SessionIdBrand | undefined
  /** Rows of the last session list shown (full or /attach scoped), for index-based selectors. */
  rows: SessionSummary[] | undefined
  /**
   * The /attach keyboard's list: every running session first, then the five
   * most recently completed ones (global across scopes, archived excluded).
   * Set when the attach keyboard is installed and cleared when a surface
   * takes its place, so numeric `/attach <n>` selectors always resolve
   * inside the keyboard's own numbered rows while it is installed.
   */
  attachKeyboard: SessionSummary[] | undefined
  /** Rows of the last workspace-keyboard output, for `/new <n>` and /attach scope selectors. */
  workspaces: WorkspaceView[] | undefined
  /**
   * The last keyboard shown was the /attach scope picker: a bare numeric
   * `/attach <n>` that follows selects the workspace row, not a session row.
   * Every session list (scoped or full) clears it, so an attach target always
   * resolves inside the last visible rows.
   */
  attachScopes: boolean
  /**
   * The reply keyboard this chat's last message shows. A delete or rename
   * re-emits a shown session list so its rows never stay stale; every other
   * surface (scope picker, workspace picker, model picker, or no keyboard)
   * clears it, so a refresh only happens for a live session list.
   */
  keyboard: VisibleKeyboard | undefined
  /** A turn of the bound session is open → drive the typing indicator. */
  typing: boolean
  /** The live message the open turn edits in place; its accumulated text. */
  stream: { messageId: number; text: string } | undefined
  /**
   * The open turn's accumulated token accounting (from `assistant/message`
   * usage records), rendered into the turn-end footer.
   */
  turnUsage: RoundUsage
  /** The open turn's per-action lines (reasoning passes and tool calls) in call order. */
  turnActions: StepAction[]
  /** The open turn's start time (the `turn/start` event time); undefined when unseen. */
  turnStartTime: number | undefined
  /**
   * A bare `/rename` asked for its title: the next non-command text applies
   * to the bound session instead of forwarding as a prompt; any `/` command
   * cancels and runs normally.
   */
  awaitingRename: boolean
  /**
   * One rendered `question/asked` ask batch: the live keyboard message and
   * the answer state the inline buttons collect. Keyed by the host's rpcId.
   */
  pendingAsks: Map<string, PendingAsk>
  /**
   * A 「✍️ 自定义回答」 button asked for free text: the next non-command text
   * becomes the custom answer of that question instead of a prompt; any `/`
   * command cancels the arm and runs normally.
   */
  awaitingAskAnswer: { rpcId: string; questionIndex: number } | undefined
  /**
   * A preset staged for the next `/new` (the bound session already started,
   * or none is bound): the created session names it in its create payload.
   */
  nextPreset: string | undefined
  /**
   * A destructive command awaiting its second send to confirm: `/close` and
   * `/archive <target>` arm it with their reply; re-sending the same command
   * executes, any other command or free text cancels.
   */
  awaitingConfirm: { command: 'close' } | { command: 'archive'; sessionId: SessionIdBrand } | undefined
}

/** One ask batch rendered for one chat; the collected answer state. */
interface PendingAsk {
  /** The host question id (the `question/asked` event id). */
  rpcId: string
  /** The session owning the ask — the answer carries it back. */
  sessionId: SessionIdBrand
  /** The inline-keyboard message this batch renders on. */
  messageId: number
  /** The questions, in order. */
  questions: AskUserQuestionItem[]
  /** Chosen option labels per question (kept in tap order). */
  selected: string[][]
  /** Captured custom answer per question; `undefined` = none yet. */
  custom: (string | undefined)[]
  /** The host settled the ask as answered; the keyboard waits for turn/end. */
  answered: boolean
}
 ...
 */
export function interpretInput(text: string): Command | { kind: 'prompt'; text: string } {
...
export class TelegramConsole {
  readonly #port: SessionConsolePort
  readonly #transport: ConsoleTransport
  readonly #now: NowFn
  readonly #chats = new Map<number, ChatState>()
  /**
   ...
   */
  constructor(port: SessionConsolePort, transport: ConsoleTransport, now: NowFn = () => Date.now()) {
    this.#port = port
   ...
   */
  async onSessionEvent(sessionId: SessionIdBrand, event: SessionEvent): Promise<void> {
  ...
  async #pushEvent(
    ...
    switch (event.type) {
      ...
      case 'turn/end': {
        ...
        // text alone.
        const label =
          reason.kind === 'completed' && stream !== undefined && stream.text !== ''
            ? undefined
          ...
          try {
            if (stream !== undefined) {
              ...
              await this.#transport.editMessage(
                chatId,
                stream.messageId,
                label === undefined ? stream.text : stream.text === '' ? label : `${stream.text}\n${label}`,
              )
            } else {
            ...
            if (label !== undefined) await this.#reply(chatId, label)
          }
          break
        }
   ...
   */
  async handleCallback(chatId: number, data: string): Promise<void> {
    const state = this.#state(chatId)
    ...
    if (list !== undefined) {
      try {
        if (list.kind === 'stop') {
          ...
          if (details !== undefined) await this.#reply(chatId, details)
        }
      } catch (error: unknown) {
        await this.#reply(chatId, `⛔ ${error instanceof Error ? error.message : String(error)}`)
      }
  ...
  async #renderAsk(
    ...
    const messageId = await this.#transport.sendInlineKeyboard(
      chatId,
      questionMessageText(questions),
      questionKeyboard(questions, questions.map(() => []), questions.map(() => undefined), rpcId),
    )
   ...
   */
  async #settleAsk(chatId: number, state: ChatState, rpcId: string, outcome: 'answered' | 'cancelled'): Promise<void> {
    const pending = state.pendingAsks.get(rpcId)
    if (pending === undefined) return
    if (state.awaitingAskAnswer?.rpcId === rpcId) state.awaitingAskAnswer = undefined
    if (outcome === 'cancelled') {
      await this.#transport.editInlineKeyboard(
        chatId,
        pending.messageId,
        '🚫 已取消。',
        [],
  ...
  async #refreshAskKeyboard(chatId: number, pending: PendingAsk): Promise<void> {
    await this.#transport.editInlineKeyboard(
      chatId,
      pending.messageId,
      questionMessageText(pending.questions),
      questionKeyboard(pending.questions, pending.selected, pending.custom, pending.rpcId),
    )
  }
   ...
   */
  async #armCustom(chatId: number, state: ChatState, pending: PendingAsk, questionIndex: number): Promise<void> {
    const question = pending.questions[questionIndex]
   ...
   */
  async #submitAsk(chatId: number, pending: PendingAsk): Promise<void> {
    ...
    if (incomplete !== -1) {
      ...
      await this.#reply(
        chatId,
        `⛔ 还有问题「${truncate(question.question, 40)}」未回答：选择选项或点「✍️ 自定义回答」。`,
      )
      return
    }
    await this.#port.answerQuestion(pending.rpcId, pending.sessionId, {
      answers: answers as { id: string; selected: string[]; custom?: string }[],
    })
  }
   ...
   */
  async #cancelAsk(chatId: number, state: ChatState, pending: PendingAsk): Promise<void> {
    await this.#port.cancelQuestion(pending.rpcId)
    if (state.awaitingAskAnswer?.rpcId === pending.rpcId) state.awaitingAskAnswer = undefined
   ...
   */
  async #handleAskCustomText(
    chatId: number,
    state: ChatState,
    awaiting: { rpcId: string; questionIndex: number },
    text: string,
  ): Promise<void> {
    state.awaitingAskAnswer = undefined
  ...
  #clearAsks(state: ChatState): void {
   ...
   */
  async #handlePendingRename(chatId: number, state: ChatState, text: string): Promise<void> {
    state.awaitingRename = false
   ...
   */
  async #handlePrompt(chatId: number, state: ChatState, text: string): Promise<void> {
    const sessionId = state.sessionId
    if (sessionId === undefined) {
  ...
  async #handleCommand(chatId: number, state: ChatState, command: Command): Promise<void> {
   ...
   * @returns after the keyboard message dispatches.
   */
  async #sessionListKeyboard(chatId: number, state: ChatState, hint: string): Promise<void> {
   ...
   * @returns after the keyboard message dispatches.
   */
  async #sessionListInline(chatId: number, state: ChatState, hint: string, verb: 'stop' | 'status'): Promise<void> {
    const items = await this.#port.listSessions()
    const visible = items.filter(item => !item.blank && item.origin !== 'subagent')
    state.rows = visible
    state.attachScopes = false
    ...
    const body = visible
      .slice(0, SESSION_KEYBOARD_MAX)
      .map((item, index) => sessionRow(index + 1, item, now)).join('\n')
    await this.#transport.sendInlineKeyboard(
      chatId,
      `${hint}\n\n${body}\n\n${SESSION_STATE_LEGEND}`,
      sessionActionButtons(visible, verb),
    )
  }
   ...
   * @returns after the keyboard message dispatches.
   */
  async #workspaceListKeyboard(chatId: number, state: ChatState, hint: string): Promise<void> {
   ...
   */
  #resolveWorkspace(state: ChatState, raw: string): WorkspaceView | undefined {
    /* v8 ignore start --
   ...
   */
  async #partitionSessions(): Promise<{
    workspaces: WorkspaceView[]
    archived: SessionSummary[]
    ungrouped: SessionSummary[]
    byWorkspace: Map<string, SessionSummary[]>
  }> {
    ...
    for (const item of visible) {
      ...
      if (bucket === undefined) byWorkspace.set(workspace.workspaceId, [item])
   ...
   */
  async #commandAttach(chatId: number, state: ChatState, args: string): Promise<void> {
    await this.#resetActionRows(chatId, state)
   ...
   */
  async #performAttach(chatId: number, state: ChatState, sessionId: SessionIdBrand): Promise<void> {
  ...
  async #attachScopePicker(chatId: number, state: ChatState): Promise<void> {
    ...
    await this.#transport.sendInlineKeyboard(
      chatId,
      `${header}\n\n${body}`,
      attachScopeButtons(workspaces, { ungrouped: ungrouped.length > 0, archived: archived.length > 0 }),
    )
  }
   ...
   */
  async #attachKeyboardItems(): Promise<SessionSummary[]> {
   ...
   * @returns after the keyboard message dispatches.
   */
  async #attachKeyboardInstall(chatId: number, state: ChatState): Promise<void> {
    const items = await this.#attachKeyboardItems()
    if (items.length === 0) return
    state.attachKeyboard = items
    state.rows = items
    await this.#transport.sendReplyKeyboard(
      chatId,
      '🔗 快捷绑定：运行中的全部会话，加最近完成的 5 个。点按钮直接绑定（或 /attach <序号>）。',
      attachKeyboardRows(items),
    )
  }

  async #attachUngrouped(chatId: number, state: ChatState): Promise<void> {
    const { ungrouped } = await this.#partitionSessions()
    await this.#attachList(chatId, state, '未分组会话：', ungrouped, { list: 'ungrouped' })
  }

  async #attachArchived(chatId: number, state: ChatState): Promise<void> {
    const { archived } = await this.#partitionSessions()
    await this.#attachList(chatId, state, '归档会话：', archived, { list: 'archived' })
  }

  async #attachWorkspaceSessions(chatId: number, state: ChatState, workspaceId: WorkspaceId): Promise<void> {
    ...
    await this.#attachList(
      chatId, state,
      `工作区「${workspace.title}」的会话：`,
   ...
   */
  async #commandClose(chatId: number, state: ChatState): Promise<void> {
   ...
   * @returns after the keyboard message dispatches.
   */
  async #commandKeyboard(chatId: number, state: ChatState): Promise<void> {
  ...
  async #resetActionRows(chatId: number, state: ChatState): Promise<void> {
  ...
  async #commandStop(chatId: number, state: ChatState, args: string): Promise<void> {
    await this.#resetActionRows(chatId, state)
  ...
  async #commandStatus(chatId: number, state: ChatState, args: string): Promise<void> {
    await this.#resetActionRows(chatId, state)
    const [rawTarget] = splitArgs(args)
    if (rawTarget === '') {
      const bound = state.sessionId
   ...
   */
  async #statusDetails(chatId: number, id: SessionIdBrand): Promise<string | undefined> {
    const items = await this.#port.listSessions()
   ...
   */
  async #commandModel(chatId: number, state: ChatState, args: string): Promise<void> {
   ...
   * @returns after the keyboard message dispatches.
   */
  async #modelListKeyboard(chatId: number, state: ChatState): Promise<void> {
    const catalog = await this.#port.listGlobalModels()
    const buttons = catalog.groups.flatMap(group =>
      group.models.map(model => `/model ${group.id}/${model.id}`))
    if (buttons.length === 0) {
      state.keyboard = undefined
      state.attachKeyboard = undefined
   ...
   */
  async #commandCreate(chatId: number, state: ChatState): Promise<void> {
    state.keyboard = { kind: 'create' }
    state.attachKeyboard = undefined
    await this.#transport.sendReplyKeyboard(
      chatId,
      '创建会话：选择一种方式。\n\n· /new — 新建会话（选工作区 / 未分类 / 路径）\n· /fork — 分叉当前会话（从最后一个已完成回合）',
      [['/new', '/fork']],
    )
  }
   ...
   * @returns after the menu dispatches.
   */
  async #commandOperate(chatId: number, state: ChatState): Promise<void> {
    state.keyboard = { kind: 'operate' }
    state.attachKeyboard = undefined
    await this.#transport.sendReplyKeyboard(
      chatId,
      '操作会话：选择一项。\n\n· /archive — 归档当前会话\n· /stop — 停止进行中的回合\n· /curTasks — 查看任务列表',
      [['/archive'], ['/stop'], ['/curTasks']],
    )
  }
   ...
   */
  async #commandNew(chatId: number, state: ChatState, args: string): Promise<void> {
    const [rawTarget] = splitArgs(args)
    if (rawTarget === '') {
    ...
    try {
      ...
      if (rawTarget === 'none') {
        ...
        sessionId = await this.#port.createSession(
          preset === undefined ? { workspaceId: workspace.workspaceId } : { workspaceId: workspace.workspaceId, agentPreset: preset },
        )
      ...
      await this.#reply(
        chatId,
        `🔗 已创建新会话 ${sessionId}（${shortSessionId(sessionId)}）· ${location}${presetNote}`,
      )
    } catch (error: unknown) {
      await this.#reply(chatId, `⛔ ${error instanceof Error ? error.message : String(error)}`)
    }
   ...
   */
  async #commandArchive(chatId: number, state: ChatState, args: string): Promise<void> {
    await this.#resetActionRows(chatId, state)
    const [rawTarget] = splitArgs(args)
   ...
   */
  async #commandDelete(chatId: number, state: ChatState, args: string): Promise<void> {
    const [rawTarget] = splitArgs(args)
    if (rawTarget === '') {
   ...
   */
  async #commandRename(chatId: number, state: ChatState, args: string): Promise<void> {
    await this.#resetActionRows(chatId, state)
    const raw = args.trim()
    if (raw === '') {
      const bound = state.sessionId
   ...
   */
  async #commandFork(chatId: number, state: ChatState, args: string): Promise<void> {
    await this.#resetActionRows(chatId, state)
    const [rawTarget] = splitArgs(args)
    ...
    try {
      ...
      await this.#reply(
        chatId,
        `🔀 已分叉会话 ${shortSessionId(resolved.id)}：新会话 ${childId}（${shortSessionId(childId)}），已绑定到新会话。`,
      )
    } catch (error: unknown) {
      await this.#reply(chatId, `⛔ ${error instanceof Error ? error.message : String(error)}`)
    }
   ...
   */
  async #commandCurTasks(chatId: number, state: ChatState): Promise<void> {
    await this.#resetActionRows(chatId, state)
    const sessionId = state.sessionId
    if (sessionId === undefined) {
   ...
   */
  async #commandPreset(chatId: number, state: ChatState, args: string): Promise<void> {
    const [rawTarget] = splitArgs(args)
    if (rawTarget === '') {
   ...
   */
  async #applyPreset(chatId: number, state: ChatState, presetId: string, label: string): Promise<void> {
    const sessionId = state.sessionId
    if (sessionId === undefined) {
    ...
    try {
      ...
      if (message.includes('（agent-preset-locked）')) {
        ...
        await this.#reply(
          chatId,
          `当前会话已开始，模式已固定。已暂存「${label}」：下一次 /new 创建会话时生效。`,
        )
        return
      }
   ...
   * @returns after the keyboard message dispatches.
   */
  async #presetListKeyboard(chatId: number, state: ChatState): Promise<void> {
    const presets = await this.#port.listPresets()
    if (presets.length === 0) {
      state.keyboard = undefined
      state.attachKeyboard = undefined
   ...
   */
  async #refreshSessionList(chatId: number, state: ChatState): Promise<void> {
    const keyboard = state.keyboard
   ...
   */
  #findModel(catalog: { groups: ModelProviderGroup[] }, name: string): {
    provider: string
    providerName: string
    model: string
    modelName: string
   ...
   */
  #looksLikeSelector(token: string): boolean {
  ...
  #resolveArgOrBound(
    ...
    raw: string,
    verb: string,
  ): { ok: true; id: SessionIdBrand } | { miss: string } {
    if (raw.trim() === '') {
      if (state.sessionId !== undefined) return { ok: true, id: state.sessionId }
   ...
   * @returns the resolved session id, or a user-facing miss message.
   */
  #resolveTarget(state: ChatState, raw: string): { ok: true; id: SessionIdBrand } | { miss: string } {
    if (/^\d+$/.test(raw)) {
   ...
   */
  async #publishAssistant(chatId: number, state: ChatState, block: string): Promise<void> {
    const stream = state.stream
 ...
 */
function turnOutcomeLabel(reason: TurnEndReason): string {
